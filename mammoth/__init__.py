...
def convert(
    ...
    **kwargs
    ...
    return options.read_options(kwargs).bind(lambda convert_options:
        docx.read(fileobj, external_file_access=external_file_access, track_changes=convert_options.pop("track_changes", False), add_page_breaks=convert_options.pop("add_page_breaks", False)).map(transform_document).bind(lambda document:
